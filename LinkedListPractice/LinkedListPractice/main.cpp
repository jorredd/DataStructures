...
int main()
{
	...
	string wait = "wait";
	string insert = "Gets put after it finds hello";
	string deleteThis = "This will be deleted";

	addNode(Head, hello);
	addNode(Head, wait);
	addNode(Head, goodbye);
	addNode(Head, deleteThis);
	insertAfter(hello, insert , Head);
	insertAfter("fail", insert , Head);

	printList(Head);
	//Searches through
	deleteNode(deleteThis, Head);
	return 0;
};
